...
v221/ , v230/    generated: types + clients + server handlers + roles + schema
...
- **v221 / v230** are generated. All of a version's types live in **one package**
  because OCPI modules reference each other's schemas cyclically (which Go
  forbids across packages); type names are globally unique, so a single namespace
  is collision-free. The package also holds typed Sender/Receiver clients,
  `RegisterXxx` server wiring, `RegisterCPO/MSP/Hub` role presets, and an embedded
...
version is mostly vendoring its spec and running `make generate`.

...
func TestSingleTariff_FlatPerDistinctElement(t *testing.T) {
	d := decimal.RequireFromString
	...
	tariff := Tariff{
		...
		Elements: []Element{
			{
				Restrictions: &Restrictions{StartTime: sp("09:00"), EndTime: sp("09:30")},
				Components: []PriceComponent{
					{Type: Flat, Price: d("1.00"), StepSize: 0},
					{Type: Energy, Price: d("0.00"), StepSize: 1},
				},
			},
			{
				Restrictions: &Restrictions{StartTime: sp("09:30"), EndTime: sp("10:00")},
				Components: []PriceComponent{
	...
	in := mtInput(
		...
		[]Period{
			...
			{Start: start.Add(30 * time.Minute), Energy: decimalPtr(d("1.0")), TariffIndex: intPtr(0)},
		},
		start, start.Add(time.Hour),
	)
	...
	// Spec section 2 charges FLAT once per distinct active FLAT tariff element.
...
func TestMultiTariff_MinMaxAppliedWhenOnlyOneTariffUsed(t *testing.T) {
	d := decimal.RequireFromString
	start := time.Date(2026, 6, 24, 9, 0, 0, 0, time.UTC)
	tariffA := mtEnergyTariff("a", d("0.30"), 1)
	tariffA.MinPrice = &Money{BeforeTaxes: d("5.00")}
	...
	assert.True(t, rep.TotalCost.BeforeTaxes.Equal(d("5.00")), "got %s", rep.TotalCost.BeforeTaxes)
	assert.False(t, hasWarn(rep.Warnings, WarnMinMaxUndefinedMultiTariff), "warnings: %#v", rep.Warnings)
	require.True(t, hasWarn(rep.Warnings, WarnUnusedTariff), "warnings: %#v", rep.Warnings)
	for _, w := range rep.Warnings {
		if w.Code == WarnUnusedTariff {
...
func TestMultiTariff_TariffWindowWarnsForUsedTariffsOnly(t *testing.T) {
	d := decimal.RequireFromString
	start := time.Date(2026, 6, 24, 9, 0, 0, 0, time.UTC)
	tariffA := mtEnergyTariff("a", d("0.30"), 1)
	tariffB := mtEnergyTariff("b", d("0.50"), 1)
	windowStart := start.Add(time.Hour)
	tariffA.StartDateTime = &windowStart
	tariffB.StartDateTime = &windowStart
	in := mtInput(
		[]Tariff{tariffA, tariffB},
		[]Period{{Start: start, Energy: decimalPtr(d("1.0")), TariffIndex: intPtr(0)}},
	...
	windowWarns := 0
	for _, w := range rep.Warnings {
		if w.Code == WarnTariffWindow {
			windowWarns++
			require.NotNil(t, w.TariffIndex)
			assert.Equal(t, 0, *w.TariffIndex)
		}
	}
	assert.Equal(t, 1, windowWarns, "warnings: %#v", rep.Warnings)
	require.True(t, hasWarn(rep.Warnings, WarnUnusedTariff), "warnings: %#v", rep.Warnings)
}

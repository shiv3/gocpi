...
export async function calculateWithTariff(
  version: Version,
  cdr: unknown,
  tariff: unknown,
  opts: EngineOptions,
  ...
  await ensure()
  const raw = (window as any).gocpiCalculateWithTariff(version, toCdrJson(cdr), JSON.stringify(tariff), JSON.stringify(opts))
  return JSON.parse(raw) as CalculateResponse
}
...
export async function verifyWithTariff(
  version: Version,
  cdr: unknown,
  tariff: unknown,
  opts: EngineOptions,
  ...
  await ensure()
  const raw = (window as any).gocpiVerifyWithTariff(version, toCdrJson(cdr), JSON.stringify(tariff), JSON.stringify(opts))
  return JSON.parse(raw) as VerifyResponse
}
